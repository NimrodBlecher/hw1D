...
AVLnode<Data,Key>* mergeTrees(AVLnode<Data,Key>* root1,AVLnode<Data,Key>* root2) {
    ...
    cout << "num1 is " << num_of_nodes1 << "and numb 2 is : " << num_of_nodes2;// <----------------------- calculate the new tree height
...
template <class Data, class Key>
void merge_ordered_arrays(AVLnode<Data, Key>* source1,AVLnode<Data, Key>* source2,AVLnode<Data, Key>* destination);
...
template <class Data, class Key>
AVLnode<Data,Key>* AVLnode<Data,Key>::insertInOrder(AVLnode<Data, Key>** array, int* start,AVLnode<Data, Key>* root )
{
    cout<< "im here" << endl;
    if(root == nullptr)
    {
...
template <class Data, class Key>
AVLnode<Data,Key>* AVLnode<Data,Key>::find (Key key) {
    AVLnode<Data,Key> *find_node = this;
    while (find_node != nullptr) {
        if (find_node-> key1 > key) {
            if (find_node->left == nullptr) {
                return nullptr;
            }
            find_node = find_node->left;
            continue;
        } else if (find_node-> key1 < key) {
...
template <class Data, class Key>
AVLnode<Data, Key>* AVLnode<Data, Key>:: deleteNode(Key key) {
    AVLnode<Data, Key> *find_node = this;
    while (find_node != nullptr) {
        if (find_node->key1 > key) {
            if (find_node->left == nullptr) {
...
template <class Data, class Key>
AVLnode<Data,Key>** AVLnode<Data, Key>:: inOrderToArray(AVLnode<Data, Key>* array[],int* start,AVLnode<Data,Key>* root)
{
    if(root == nullptr)
    {
        return NULL;
